...
class Fighter:
    def __init__(self, x,y,w,h,health,img, slice_w, slice_h, shift,r):
        ...
        self.r = r
        #self.action_triggered = {'kick': False, 'punch': False}
    ...
    def display(self):
        ...
        if self.dir == RIGHT:
            if self.walking:
                self.num_slices = 6
                image(self.img, self.x - self.shift, self.y , self.slice_w, self.slice_h, 125 * 8 + self.slice * self.slice_w, 0, 125 * 8 + (self.slice + 1)* self.slice_w, self.slice_h)
            elif self.punching:
                self.num_slices = 10
                image(self.img, self.x - self.shift, self.y, self.slice_w, self.slice_h,  125*5 + self.slice * self.slice_w, 0, 125*5 + (self.slice + 1) * self.slice_w, self.slice_h )
            elif self.kicking == True:
                self.num_slices = 4
                image(self.img, self.x - self.shift, self.y , self.slice_w, self.slice_h,125*16 + self.slice * self.slice_w, 0, 125 * 16 +(self.slice + 1)* self.slice_w, self.slice_h)
            elif self.blocking == True:
                self.num_slices = 6
                image(self.img, self.x - self.shift, self.y , self.slice_w, self.slice_h, 125 * 20 + self.slice * self.slice_w, 0, 125 * 20 + (self.slice + 1)* self.slice_w, self.slice_h)
        elif self.dir == LEFT:
            if self.walking:
                self.num_slices = 7
                image(self.img, self.x - self.shift  , self.y , self.slice_w, self.slice_h, 125 * 8 + (self.slice + 1) * self.slice_w, 0,125 * 8 + self.slice * self.slice_w, self.slice_h)
            if self.punching:
                self.num_slices = 10
                image(self.img, self.x - self.shift, self.y, self.slice_w, self.slice_h, 125*5 + (self.slice + 1) * self.slice_w, 0,  125*5 + self.slice * self.slice_w, self.slice_h )    
            elif self.kicking:
                self.num_slices = 4
                image(self.img, self.x - self.shift, self.y , self.slice_w, self.slice_h, 125 *16 + (self.slice + 1) * self.slice_w, 0, 125 *16 + self.slice * self.slice_w, self.slice_h)
            elif self.blocking:
                self.num_slices = 6
                image(self.img, self.x - self.shift, self.y , self.slice_w, self.slice_h, 125 * 20 + (self.slice + 1) * self.slice_w, 0, 125 * 20 + self.slice * self.slice_w, self.slice_h)
   #checks if the fighters are colliding,if the fighter is blocking the attack from the right direction or is jumping while recieving the attack, then he is protected
    ...
    def move(self):
       ...
       #collision
        if Games[-1].Collide:
            if Games[-1].fighter_1.x < Games[-1].fighter_2.x and Games[-1].fighter_1.vy != 0:
                Games[-1].fighter_1.vx -= 7
            if Games[-1].fighter_1.x > Games[-1].fighter_2.x and Games[-1].fighter_1.vy != 0:
                Games[-1].fighter_1.vx += 7
                
            if Games[-1].fighter_2.x < Games[-1].fighter_1.x and Games[-1].fighter_2.vy != 0:
                Games[-1].fighter_2.vx -= 7
            if Games[-1].fighter_2.x > Games[-1].fighter_1.x and Games[-1].fighter_2.vy != 0:
                Games[-1].fighter_2.vx += 7
                
            if Games[-1].fighter_1.x < Games[-1].fighter_2.x and Games[-1].fighter_1.key_handler[RIGHT] == True:    
                Games[-1].fighter_1.vx -= 7
            if Games[-1].fighter_1.x > Games[-1].fighter_2.x and Games[-1].fighter_1.key_handler[LEFT] == True:
                Games[-1].fighter_1.vx += 7
            if Games[-1].fighter_2.x > Games[-1].fighter_1.x and Games[-1].fighter_2.key_handler[LEFT] == True:
                Games[-1].fighter_2.vx += 7
            if Games[-1].fighter_2.x < Games[-1].fighter_1.x and Games[-1].fighter_2.key_handler[RIGHT] == True:
                Games[-1].fighter_2.vx -= 7
            
        if self.key_handler[UP] == True and self.y + self.h - 20 == self.g:
            self.vy = -45
...
class Game:
    def __init__(self):
        self.fighter_1 = Fighter(200,310,125,125, 20, "C1.png", 125, 125,12, 32)
        self.fighter_2 = Fighter(700,310,125,125, RESOLUTION_X - 120,"CB.png", 125, 125, 12,32)
    ...
    def display(self):
        ...
        if ((Games[-1].fighter_1.x  - Games[-1].fighter_2.x)** 2 + (Games[-1].fighter_1.y - Games[-1].fighter_2.y)** 2)** 0.5 <= 2 * Games[-1].fighter_2.r:
            self.Collide = True 
        else:
            self.Collide = False
        print(self.Collide,Games[-1].fighter_1.x,Games[-1].fighter_2.x,Games[-1].fighter_1.y,Games[-1].fighter_2.y,Games[-1].fighter_1.r)
        self.game_over()
...
def mouseClicked():
    global GameState
    if GameState == "Start" and RESOLUTION_X/2 - 52 <=mouseX <= RESOLUTION_X/2 + 54 and RESOLUTION_Y/2 - 15 <=mouseY < RESOLUTION_Y/2 + 35 :
        GameState = "Playing"
    elif GameState in ["GameOver" , "Playing"]:
...
start_bg = loadImage(PATH + "/Sprites/" + "Opening.png")
def setup():
    size(RESOLUTION_X,RESOLUTION_Y)
def draw():
    if GameState == "Start":
        background(0)
        image(start_bg, 0, 0, RESOLUTION_X, RESOLUTION_Y) 
        # fill(255,0,0)
        # rect(RESOLUTION_X/2 - 52, RESOLUTION_Y/2 - 15, 106 , 50)
